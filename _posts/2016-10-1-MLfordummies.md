...
Leo Breiman's [2001 article](http://www.stat.uchicago.edu/~lekheng/courses/191f09/breiman.pdf) "Statistical modelling: the two cultures"  contrasts modern ML methods with traditional statistics. Some view Leo's distinction as a bit artificial - in the sense that many analysts combine methods from both schools (the term 'Data Science' is often used to describe a broader field that combines all statistical methods old and new).
...
There have been no shortage of researchers, willing to point out problems with applied econometrics (see for example [McCloskey and Zilak](http://www.deirdremccloskey.com/articles/stats/preface_ziliak.php)). 
