...
So all we've done so far is add some type declarations. For local variables we use the `cdef' keyword. For arrays we use the [memoryview](http://docs.cython.org/src/userguide/memoryviews.html) syntax.  Any code in a `*.pyx` file you want to be converted to C needs to be typed. So everything inside loops has to be typed (including the indexes). If you leave a variable untyped the code will still compile, but you won't get C speeds.
...
Not too bad, but we can do a lot better. One criticism of Cython is that there are a few 'tricks' involved in achieving good performance. Here's the first one, if we type this in the terminal 
