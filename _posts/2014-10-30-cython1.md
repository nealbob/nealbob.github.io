...
However, for many applications we can't escape the need for loops. One option is `numba` a just-in-time compiler for Python, another is to give up on Python and learn Julia (see [quantecon](http://quant-econ.net/python_or_julia.html)).  
...
so our function should take an input data array \\( X\\) of shape (N, D), a parameter array \\( \beta\\) of length N and a 'bandwidth' parameter \\(\theta\\) and return an array of fitted values \\(  \hat Y \\) of length N. 
