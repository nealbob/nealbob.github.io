...
So all we've done here is add some type declarations. For arrays we use the `memoryview' syntax.  Any code in a `*.pyx` file that you want to be converted to C needs to be typed. So everything inside loops has to be typed (including the indexes). If you leave a variable untyped the code will still compile, but you won't get C speeds.
