...
So far all we've done so far is add some type declarations. For local variables we use the `cdef` keyword. For arrays we use ['memoryviews'](http://docs.cython.org/src/userguide/memoryviews.html) which can accept numpy arrays as input.  
