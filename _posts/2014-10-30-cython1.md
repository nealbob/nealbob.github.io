...
so we need a function which takes an input data array <span>\\(\mathbf X\\)<span> of shape (N, D), a parameter array \\(\mathbf \beta\\) of length N and a 'bandwidth' parameter \\(\theta\\) and returns the array of fitted values \\( \mathbf \hat Y \\) of length N. 
