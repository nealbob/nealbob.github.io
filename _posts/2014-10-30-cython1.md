...
However, for many applications you can't escape the need for loops. One option is `numba` a just-in-time compiler for Python, another is to give up on Python and learn Julia (see [quantecon](http://quant-econ.net/python_or_julia.html)).  
...
Now if we run `cython fastloop.pyx -a' again we will see the loops are now free of any Python 
