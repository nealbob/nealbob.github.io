...
Pivotal has a declarative syntax similar to SQL while incorporating aspects of Python (pandas) and R (dplyr) grammar. The below example compares Pivotal to equivalent Python code for some basic data wrangling in a Jupyter Notebook.
...
*Pivotal vs Python syntax — for more detail see the [comparison page](https://nealhughes.net/pivotal-py/comparison/)*
...
Pandas limitations have been well-documented, most notably by its creator [Wes McKinney](https://wesmckinney.com/blog/apache-arrow-pandas-internals/). Firstly, there are the quirks: including indexes which most people try to avoid, leading to boilerplate like `.reset_index()` or `as_index=False`. Then there are the long standing performance limitations (lessened somewhat by recent updates).

There are of course alternatives. Polars offers better performance and has no indexes, but the syntax is even more verbose (see above example). And if we are being honest, the R [tidyverse](https://tidyverse.org/) probably offers a better experience for interactive data work than anything in Python (but it's in R).
...
The longevity of SQL tells us something about the utility of DSLs for data work. In recent years, SQL has become better integrated with Python through libraries like DuckDB, while new piped-SQL syntax including [PRQL](https://prql-lang.org/) offers a more linear Python/R style of working.
...
While Pivotal is young, the scope is reasonably extensive — covering all the common Pandas/SQL data operations, along with more complex tasks like window functions and date and string manipulation (see the [docs](https://nealhughes.net/pivotal-py/)). Pivotal also includes commands for producing outputs: plots (via `matplotlib`), tables (via [Great Tables](https://posit-dev.github.io/great-tables/articles/intro.html)) and saving to [Frictionless data packages](https://specs.frictionlessdata.io/data-package/). For anything that can't be done in Pivotal there is an easy Python escape hatch.

Pivotal has been built with Jupyter Notebooks in front of mind. The JupyterLab extension includes `%%pivotal` cell magic with syntax highlighting and context-aware auto-complete, along with GUI features including an interactive object viewer (with [AG Grid](https://www.ag-grid.com/) spreadsheets and table and plot previews). The VS Code extension offers much the same functionality.
...
**Read the docs:** [nealhughes.net/pivotal-py](https://nealhughes.net/pivotal-py/)

**Join the discussion on GitHub:** [github.com/nealbob/pivotal-py/discussions](https://github.com/nealbob/pivotal-py/discussions)
